...
from path_datasets_and_weights import IMAGE_MODE
...
class ClsGen(nn.Module):
    def __init__(self, classifier, generator, num_topics, embed_dim):
        ...
        self.lambda_layer0 = nn.Linear(256,256)
        self.lambda_layer1 = nn.Linear(256,256)
        self.label_embedding = nn.Embedding(num_topics, embed_dim)
        if IMAGE_MODE=='unbias':
            self.cls_model_origin = classifier[0]
            self.cls_model_mask = classifier[1]
        else:
            self.classifier = classifier

    def remove_feature(self, original_feature, feature_to_remove):
        """
        从 original_feature 中去除 feature_to_remove。
    
        Args:
        original_feature (torch.Tensor): 原始特征向量，形状为 (N, D)。
        feature_to_remove (torch.Tensor): 要去除的特征向量，形状为 (N, D)。
        
        Returns:
        torch.Tensor: 去除后的特征向量，形状为 (N, D)。
        """
        # 计算 feature_to_remove 的单位向量
        unit_vector = feature_to_remove / feature_to_remove.norm(dim=1, keepdim=True)
        
        # 计算 original_feature 在 feature_to_remove 方向上的投影
        projection = (original_feature * unit_vector).sum(dim=1, keepdim=True) * unit_vector
        
        # 去除投影
        result_feature = original_feature - projection
    ...
    def forward(self, image, history=None, caption=None, label=None, threshold=0.15, bos_id=1, eos_id=2, pad_id=3, max_len=300, get_emb=False):
        ...
        if IMAGE_MODE=='unbias':
            # print(image[1].shape)
            img_mlc, image0 = self.cls_model_origin(img=image, txt=history, lbl=label, threshold=threshold, pad_id=pad_id, get_embed=True) # (B,T,C), (B,T,E)
            _, image1 = self.cls_model_mask(img=image, txt=history, lbl=label, threshold=threshold, pad_id=pad_id, get_embed=True) # (B,T,C), (B,T,E)
            
            image1.requires_grad=True
            img_emb = self.remove_feature(image0,image1)
            # img_emb = self.lambda_layer0(image0)-self.lambda_layer1(image1)
            # img_emb = image0
            # img_mlc, _ = self.classifier(img=image[0], txt=history, lbl=label, threshold=threshold, pad_id=pad_id, get_embed=True) # (B,T,C), (B,T,E)
        else:
