...
class AlignHead(torch.nn.Module):
    def __init__(self,hidden_size=2048,ent_embedding_only=256):
        super().__init__()
        ...
        self.img_linear1 = nn.Linear(hidden_size, hidden_size)
        self.img_linear2 = nn.Linear(hidden_size, self.ent_hidden_size)
        self.txt_linear1 = nn.Linear(hidden_size, hidden_size)
        self.txt_linear2 = nn.Linear(hidden_size, self.ent_hidden_size)
        self.linear = nn.Linear(1043,64)

        self.ent_embedding_only = ent_embedding_only
...
class Transformer(nn.Module):
    def __init__(self, image_encoder, num_tokens, num_posits, fc_features=1024, embed_dim=256, num_heads=8, fwd_dim=4096, dropout=0.1, num_layers_enc=1, num_layers_dec=6, freeze_encoder=True):
        ...
        self.knowledge_embedding = nn.Embedding(64, embed_dim)
        ...
        self.fc3 = nn.Linear(1043,64)
        self.alignhead = AlignHead()
    ...
    def forward(self, image, entity=None, caption = None, bos_id=1, eos_id=2, pad_id=3, max_len=300):
        if caption != None:
            avg_features, wxh_features = self.image_encoder(image)  # (B,F), (B,F,W,H)
            avg_features = self.alignhead.img_classify(avg_features)

            ent_embedding = self.alignhead.embeddings(entity)
            ent_embedding = self.fc3(ent_embedding.transpose(1,2)).transpose(1,2)


            wxh_features = wxh_features.view(wxh_features.shape[0], wxh_features.shape[1], -1).permute(0, 2, 1)  # (B,W*H,F)
            wxh_features = self.fc1(wxh_features)
            wxh_features = wxh_features+ent_embedding
            ...
            self.features = wxh_features
            return preds,avg_features # (B,L,S)
            ...
            return caption,avg_features,self.features # (B,L')
